    ...
    <aside class="messenger-sidebar">
      ...
      <div class="sidebar-search">
        <form method="GET" action="{{ route('messages.index') }}">
          <input type="search" name="search" value="{{ $search ?? '' }}" placeholder="Search contacts...">
        </form>
      </div>
      ...
      <div class="sidebar-section">
        ...
        <div class="conversation-list">
          @forelse($admins as $item)
            <a href="{{ route('messages.show', $item['contact']->id) }}" class="conversation-item">
              <div class="d-flex align-items-center gap-3">
                <div class="conversation-avatar">{{ strtoupper(substr($item['contact']->name, 0, 1)) }}</div>
                <div class="conversation-details">
                  <div class="conversation-name">{{ $item['contact']->name }}</div>
                  <div class="conversation-preview">{{ $item['preview'] }}</div>
                </div>
              </div>
              <div class="conversation-meta">
                @if($item['time'])<div class="timestamp">{{ $item['time'] }}</div>@endif
                @if($item['unread'] > 0)<span class="badge-unread">{{ $item['unread'] }}</span>@endif
              </div>
            </a>
          @empty
            ...
            @if($message->attachment && preg_match('/\.(jpg|jpeg|png|gif|webp)$/i', $message->attachment))
              <div class="image-preview mt-3">
                <a href="{{ asset('storage/' . $message->attachment) }}" target="_blank">
                  <img src="{{ asset('storage/' . $message->attachment) }}" alt="Attachment preview">
                </a>
              </div>
            @elseif($message->attachment)
              <div class="message-attachment mt-3"><a href="{{ asset('storage/' . $message->attachment) }}" target="_blank"><i class="fas fa-paperclip me-2"></i>Download attachment</a></div>
            @endif
