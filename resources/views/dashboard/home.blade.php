...
[data-theme="dark"] .quick-overview-card {
...
[data-theme="dark"] .quick-overview-item {
...
[data-theme="dark"] .quick-overview-item span:first-child {
    color: #cbd5e1;
}
...
[data-theme="dark"] .quick-overview-item span:last-child {
    color: #f1f5f9;
}
...
[data-theme="dark"] .hero-status-label {
    color: #38bdf8;
}
...
[data-theme="dark"] .hero-status-value {
    color: #f1f5f9;
}
            ...
            <div class="card-modern dashboard-hero hover-lift">
                <div class="dashboard-header-grid">
                    <div class="time-widget p-4 hover-lift">
                        ...
                        <div class="widget-value" id="compactTime">{{ now()->format('g:i A') }}</div>
                        <div class="widget-meta"><i class="fas fa-calendar-day"></i> <span id="compactDate">{{ now()->format('F j, Y') }}</span></div>
                        <div class="widget-role"><i class="fas fa-user-tag"></i> {{ ucfirst($user->role) }}</div>
                    </div>
                    <div class="status-widget p-4 hover-lift">
                        ...
                        <p class="widget-note">{{ $reasonMessage }}</p>
                    </div>
                    <div class="quick-overview-card p-4 hover-lift">
                        ...
                        <div class="quick-overview-list">
                            <div class="quick-overview-item"><span>Total equipment</span><span>{{ $totalEquipment }}</span></div>
                            <div class="quick-overview-item"><span>Available</span><span>{{ $availableEquipment }}</span></div>
                            <div class="quick-overview-item"><span>Pending requests</span><span>{{ $pendingApprovals }}</span></div>
                            <div class="quick-overview-item"><span>Overdue items</span><span>{{ $overdueEquipment }}</span></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="stats-grid">
                    ...
                    <div class="stat-card stat-card-soft">
                        ...
                        <div class="stat-card-value">{{ $returnRequests }}</div>
                    ...
                    <div class="stat-card stat-card-soft">
                        ...
                        <div class="stat-card-value">{{ $incidentReports }}</div>
                    ...
                    <div class="stat-card stat-card-soft">
                        ...
                        <div class="stat-card-value">{{ $overdueEquipment }}</div>
        ...
        <div class="col-xl-8">
                    ...
                    <div class="col-sm-6">
                        <div class="mini-metric-card bg-blue-soft">
                            ...
                            <div>
                                ...
                                <div class="mini-metric-value">{{ $pendingApprovals }}</div>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-6">
                        <div class="mini-metric-card bg-warning-soft">
                            ...
                            <div>
                                ...
                                <div class="mini-metric-value">{{ $returnRequests }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                                ...
                                <div class="col-sm-6">
                                    <div class="mini-metric-card bg-blue-soft h-100">
                                        ...
                                        <div>
                                            ...
                                            <div class="mini-metric-value">{{ $recentBorrowRequestsCount ?? 0 }}</div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <div class="mini-metric-card bg-warning-soft h-100">
                                        ...
                                        <div>
                                            ...
                                            <div class="mini-metric-value">{{ $activeUsers }}</div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <div class="mini-metric-card bg-blue-soft h-100">
                                        ...
                                        <div>
                                            ...
                                            <div class="mini-metric-value">{{ $availableEquipment }}</div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <div class="mini-metric-card bg-warning-soft h-100">
                                        ...
                                        <div>
                                            ...
                                            <div class="mini-metric-value">{{ $recentIncidentReportsCount ?? 0 }}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="card-modern p-4 hover-lift mt-4">
                ...
                <div class="timeline-card">
                    @forelse($borrowings->take(6) as $borrowing)
                        <div class="timeline-step">
                            <div class="timeline-dot bg-{{ $borrowing->status === 'approved' ? 'success' : ($borrowing->status === 'pending' ? 'warning' : ($borrowing->status === 'overdue' ? 'danger' : 'secondary')) }}"></div>
                            <div>
                                <div class="d-flex justify-content-between align-items-start gap-3 mb-1">
                                    <div>
                                        <div class="fw-semibold">{{ $borrowing->equipment->name }}</div>
                                        <small class="text-muted">{{ optional($borrowing->borrow_date)->format('M d, Y') ?? 'N/A' }}</small>
                                    </div>
                                    <span class="badge badge-pill badge-{{ $borrowing->status === 'approved' ? 'success' : ($borrowing->status === 'pending' ? 'warning' : ($borrowing->status === 'overdue' ? 'danger' : 'secondary')) }}">{{ ucfirst($borrowing->status) }}</span>
                                </div>
                                <p class="text-muted mb-2">{{ Str::limit($borrowing->purpose ?? 'No purpose provided', 90) }}</p>
                                <div class="d-flex align-items-center gap-2">
                                    <span class="timeline-avatar">{{ strtoupper(substr(optional($borrowing->student)->name ?? $user->name, 0, 1)) }}</span>
                                    <small class="text-muted">{{ optional($borrowing->borrow_date)->diffForHumans() ?? 'Just now' }}</small>
                                </div>
                            </div>
                        </div>
                    @empty
        ...
        <div class="col-xl-4">
            <div class="card-modern p-4 hover-lift mb-4">
                ...
                <div class="quick-action-grid">
                    @if($user->role === 'staff')
                        <a href="{{ route('staff.pending-requests') }}" class="quick-action-card">
                        ...
                        <a href="{{ route('borrowings.return-equipment') }}" class="quick-action-card">
                    ...
                    @else
                        ...
                        <a href="{{ route('bug-reports.create') }}" class="quick-action-card">
            ...
            <div class="card-modern p-4 hover-lift">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <a href="{{ route('notifications.index') }}" class="text-primary small">See all</a>
                </div>
                <div class="notifications-preview">
                    @forelse($notifications as $notification)
                        <div class="notification-preview-item {{ $notification->is_read ? '' : 'unread' }}">
                            <div class="notification-avatar">{{ strtoupper(substr($notification->title ?? 'N', 0, 1)) }}</div>
                            <div class="grow">
                                <div class="d-flex justify-content-between align-items-start gap-2 mb-1">
                                    <div>
                                        <div class="notification-title">{{ Str::limit($notification->title ?? 'Notification', 45) }}</div>
                                    </div>
                    ...
                    <table class="table table-hover table-borderless align-middle mb-0">
                        ...
                        <tbody>
                            @forelse($borrowings->take(5) as $borrowing)
                                <tr>
                                    <td><span class="fw-semibold">{{ $borrowing->equipment->name }}</span></td>
                                    <td><span class="badge bg-{{ $borrowing->status === 'approved' ? 'success' : ($borrowing->status === 'pending' ? 'warning' : 'secondary') }} bg-opacity-15 text-{{ $borrowing->status === 'approved' ? 'success' : ($borrowing->status === 'pending' ? 'warning' : 'secondary') }} rounded-pill">{{ ucfirst($borrowing->status) }}</span></td>
                    ...
                    <table class="table table-hover table-borderless align-middle mb-0">
                        ...
                        <tbody>
                            @forelse($incidents->take(5) as $incident)
                                <tr>
                                    <td><span class="fw-semibold">{{ $incident->type }}</span></td>
                                    <td><span class="badge bg-{{ $incident->severity === 'high' ? 'danger' : ($incident->severity === 'medium' ? 'warning' : 'info') }} bg-opacity-15 text-{{ $incident->severity === 'high' ? 'danger' : ($incident->severity === 'medium' ? 'warning' : 'info') }} rounded-pill">{{ ucfirst($incident->severity) }}</span></td>
                                    <td><span class="badge bg-{{ $incident->status === 'closed' ? 'success' : 'warning' }} bg-opacity-15 text-{{ $incident->status === 'closed' ? 'success' : 'warning' }} rounded-pill">{{ ucfirst($incident->status) }}</span></td>
                                </tr>
                            @empty
                                <tr>
