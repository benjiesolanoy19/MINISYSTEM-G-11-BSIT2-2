...
<form method="POST" action="{{ route('equipment.update', request()->route('id')) }}" enctype="multipart/form-data" x-data="equipmentForm($wire)" @submit="loading = true">
        @csrf
        @method('PUT')
...
function equipmentForm(wire) {
  return {
    loading: false,
    imagePreview: null,
    currentImage: '{{ $equipment->getImageUrl() }}',
    isDragging: false,
    imageRemoved: false,
